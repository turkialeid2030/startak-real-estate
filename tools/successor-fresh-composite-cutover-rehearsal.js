...
function parseArgs(argv) {
  ...
  for (const key of ['--shadow', '--rehearsal-id', '--prepared-by', '--prepared-at']) {
...
function run(argv = process.argv.slice(2)) {
  try {
    ...
    const shadow = readBoundedRegularJson(args['--shadow'], 'shadow');
    const result = runSuccessorFreshCompositeCutoverRehearsal({
      currentRegistry: registry.parsed,
      currentRegistryContent: registry.raw,
      successorFreshShadowEvaluation: shadow.parsed,
      rehearsalId: args['--rehearsal-id'],
    ...
    return result.status === STATUS.SUCCESSOR_FRESH_CUTOVER_REHEARSAL_COMPLETED_NO_ACTIVATION ? 0 : 1;
  } catch (error) {
    process.stderr.write(`P72 configuration error: ${error.message}\n`);
    return 1;
  }
}
