...
function parseArgs(argv) {
  const allowed = new Set([
    '--activation-plan', '--candidate', '--safety-guard', '--authority-registry',
    '--expected-registry-hash', '--signed-decision', '--contract-id',
    '--prepared-by', '--prepared-at', '--output',
  ]);
  const out = {};
  const seen = new Set();
  ...
  for (const required of [
    'activation-plan', 'candidate', 'safety-guard', 'authority-registry',
    'expected-registry-hash', 'signed-decision', 'contract-id', 'prepared-by', 'prepared-at',
  ]) {
...
if (require.main === module) {
  try {
    ...
    const result = createFreshActivationChangeContract({
      currentRegistry,
      activationPlan: readBoundedJson(path.resolve(args['activation-plan']), 'FRESH_ACTIVATION_PLAN'),
      freshCompositeCandidate: readBoundedJson(path.resolve(args.candidate), 'FRESH_COMPOSITE_CANDIDATE'),
      safetyGuard: readBoundedJson(path.resolve(args['safety-guard']), 'FRESH_CUTOVER_SAFETY_GUARD'),
      ...
      signedOwnerDecision: readBoundedJson(path.resolve(args['signed-decision']), 'FRESH_SIGNED_OWNER_DECISION'),
    ...
    process.exit(result.status === STATUS.FRESH_ACTIVATION_CHANGE_CONTRACT_READY_OWNER_AUTH_VERIFIED_NOT_APPLIED ? 0 : 1);
  } catch (error) {
