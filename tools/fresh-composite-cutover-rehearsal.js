...
function parseArgs(argv) {
  const allowed = new Set(['--shadow', '--rehearsal-id', '--prepared-by', '--prepared-at', '--output']);
  const out = {};
  const seen = new Set();
  ...
  for (const required of ['shadow', 'rehearsal-id', 'prepared-by', 'prepared-at']) {
...
if (require.main === module) {
  try {
    ...
    const shadow = readBoundedJson(path.resolve(args.shadow), 'FRESH_SHADOW');
    const result = runFreshCompositeCutoverRehearsal({
      currentRegistry,
    ...
    process.exit(result.status === STATUS.FRESH_CUTOVER_REHEARSAL_COMPLETED_NO_ACTIVATION ? 0 : 1);
  } catch (error) {
