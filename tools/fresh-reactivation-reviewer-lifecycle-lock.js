...
function run(argv = process.argv.slice(2)) {
  try {
    ...
    const result = createFreshReactivationReviewerLifecycleLock({
      packet: readBoundedRegularJson(requiredArg(args, '--packet'), 'packet'),
      verifiedReview: readBoundedRegularJson(requiredArg(args, '--verified-review'), 'verified review'),
    ...
    return result.status === STATUS.FRESH_REACTIVATION_REVIEWER_LOCKED_BY_VERIFIED_REVIEW ? 0 : 1;
  } catch (error) {
    process.stderr.write(`P49 configuration error: ${error.message}\n`);
    return 1;
  }
}
