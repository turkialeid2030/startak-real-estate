...
function parseArgs(argv) {
  const allowed = new Set([
    '--registry', '--activation-contract', '--review-packet', '--reviewer-lifecycle', '--activation-plan', '--candidate',
    '--shadow', '--rehearsal', '--safety-guard', '--authority-registry', '--expected-registry-hash', '--signed-decision', '--output',
  ]);
  const out = {};
  const seen = new Set();
...
function requireSuccessorArgs(args) {
  for (const required of [
    'activation-contract', 'review-packet', 'reviewer-lifecycle', 'activation-plan', 'candidate', 'shadow', 'rehearsal',
    'safety-guard', 'authority-registry', 'expected-registry-hash', 'signed-decision',
  ]) {
    if (!args[required]) throw new Error(`missing required argument for successor fresh composite mode: --${required}`);
  }
}
...
if (require.main === module) {
  try {
    ...
    if (registryFile.parsed.activeMode === MODE.GOVERNED_COMPOSITE_BASELINE && registryFile.parsed.schemaVersion === 4) {
      requireSuccessorArgs(args);
      input.activationChangeContract = readBoundedJson(path.resolve(args['activation-contract']), 'SUCCESSOR_FRESH_ACTIVATION_CHANGE_CONTRACT');
      input.successorReviewPacket = readBoundedJson(path.resolve(args['review-packet']), 'SUCCESSOR_FRESH_REVIEW_PACKET');
      input.reviewerLifecycle = readBoundedJson(path.resolve(args['reviewer-lifecycle']), 'SUCCESSOR_FRESH_REVIEWER_LIFECYCLE');
      input.activationPlan = readBoundedJson(path.resolve(args['activation-plan']), 'SUCCESSOR_FRESH_ACTIVATION_PLAN');
      input.successorFreshCompositeCandidate = readBoundedJson(path.resolve(args.candidate), 'SUCCESSOR_FRESH_COMPOSITE_CANDIDATE');
      input.successorFreshShadowEvaluation = readBoundedJson(path.resolve(args.shadow), 'SUCCESSOR_FRESH_SHADOW');
      input.successorFreshRehearsalResult = readBoundedJson(path.resolve(args.rehearsal), 'SUCCESSOR_FRESH_REHEARSAL');
      input.safetyGuard = readBoundedJson(path.resolve(args['safety-guard']), 'SUCCESSOR_FRESH_CUTOVER_SAFETY_GUARD');
      input.successorFreshOwnerAuthorityRegistry = readBoundedJson(path.resolve(args['authority-registry']), 'SUCCESSOR_FRESH_OWNER_AUTHORITY_REGISTRY');
      input.expectedSuccessorFreshOwnerAuthorityRegistryHashSha256 = args['expected-registry-hash'];
      input.signedOwnerDecision = readBoundedJson(path.resolve(args['signed-decision']), 'SUCCESSOR_FRESH_SIGNED_OWNER_DECISION');
    }
    ...
    const pass = result.status === STATUS.LEGACY_BASELINE_VERIFIED
      || result.status === STATUS.SUCCESSOR_FRESH_COMPOSITE_BASELINE_VERIFIED_WITH_SUCCESSOR_OWNER_AUTHORIZATION;
