...
function parseArgs(argv) {
  const allowed = new Set([
    '--review-packet', '--reviewer-lifecycle', '--activation-plan', '--candidate', '--shadow', '--rehearsal', '--safety-guard',
    '--authority-registry', '--expected-registry-hash', '--signed-decision', '--contract-id', '--prepared-by', '--prepared-at', '--registry', '--output',
  ]);
  const out = {};
  const seen = new Set();
  ...
  for (const required of [
    'review-packet', 'reviewer-lifecycle', 'activation-plan', 'candidate', 'shadow', 'rehearsal', 'safety-guard',
    'authority-registry', 'expected-registry-hash', 'signed-decision', 'contract-id', 'prepared-by', 'prepared-at',
  ]) {
...
function readBoundedJson(filePath, label, fsModule = fs) {
  ...
  let parsed;
  try { parsed = JSON.parse(fsModule.readFileSync(filePath, 'utf8')); } catch (_) { throw new Error(`${label}_JSON_INVALID`); }
...
if (require.main === module) {
  try {
    ...
    const registry = readRegistry(path.resolve(args.registry || DEFAULT_REGISTRY_PATH));
    const result = createSuccessorFreshActivationChangeContract({
      ...registry,
      ...
      successorFreshCompositeCandidate: readBoundedJson(path.resolve(args.candidate), 'SUCCESSOR_FRESH_COMPOSITE_CANDIDATE'),
      ...
      safetyGuard: readBoundedJson(path.resolve(args['safety-guard']), 'SUCCESSOR_FRESH_CUTOVER_SAFETY_GUARD'),
      ...
      signedOwnerDecision: readBoundedJson(path.resolve(args['signed-decision']), 'SUCCESSOR_FRESH_SIGNED_OWNER_DECISION'),
      contractId: args['contract-id'],
      preparedByRef: args['prepared-by'],
      preparedAt: args['prepared-at'],
    });
    if (args.output) writeRestrictiveJson(path.resolve(args.output), result);
    else process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
    process.exit(result.status === STATUS.SUCCESSOR_FRESH_ACTIVATION_CHANGE_CONTRACT_READY_OWNER_AUTH_VERIFIED_NOT_APPLIED ? 0 : 1);
  } catch (error) {
