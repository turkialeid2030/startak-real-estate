...
function createUiWorkspace({ mode, defaultInputs }) {
  assertMode(mode); assertPlainObject(defaultInputs, 'defaultInputs');
...
function hydrateUiDeal({ record, defaultInputs }) {
  assertPlainObject(record, 'record'); assertPlainObject(defaultInputs, 'defaultInputs');
...
function calculateUiInvestmentState({ mode, inputs, assumptionModelVersion }) {
  assertMode(mode); assertPlainObject(inputs, 'inputs');
  const version = normalizeAssumptionModelVersion(assumptionModelVersion);
  const results = calculateInvestmentCase({ studyType: studyTypeForMode(mode), inputs, leverageEnabled: Boolean(inputs.leverageEnabled), assumptionModelVersion: version });
  const governance = mode === UI_MODE.BUILDING ? buildUiAssumptionGovernance({ assumptionModelVersion: version, financialResults: results }) : null;
  return Object.freeze({
    mode, assumptionModelVersion: version, results, governance,
...
function buildUiDisclosureViewModel({ governance, locale = 'ar-SA' }) {
  ...
  return Object.freeze({
    badge: disclosure.badge[language], assumptionModelVersion: disclosure.assumptionModelVersion,
    legacyCompatibility: disclosure.legacyCompatibility, approvedAssumptionKeys: disclosure.approvedAssumptionKeys,
    exitCapSource: disclosure.exitCapSource, exitCapNotice, exitCapInputRequired: governance.exitCapInputRequired,
    sensitivityStatus: governance.sensitivity.status, sensitivityReady: governance.sensitivityReady,
    sensitivityRenderPolicy: governance.sensitivity.renderPolicy, ...authorityView(),
  });
}
...
module.exports = {
  UI_MODE, DEFAULT_TRANSACTION_AUTHORITY, authorityView, studyTypeForMode, materializeUiAssumptions,
  createUiWorkspace, hydrateUiDeal, calculateUiInvestmentState, applyExitCapInputText,
  buildUiDisclosureViewModel, prepareNewUiDealForSave, prepareUpdatedUiDealForSave, explicitlyUpgradeUiDealToV2,
};
