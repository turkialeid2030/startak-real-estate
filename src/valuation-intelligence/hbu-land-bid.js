'use strict';
const HBU_VERSION='HBU_LAND_BID_V1';
...
function evaluateHighestAndBestUse({alternatives,landAreaSqm,requiredDeveloperMarginRate,acquisitionCostsRate=0,evidenceComplete=false}={}){
 ...
 if(!evidenceComplete)blockers.push('HBU_EVIDENCE_INCOMPLETE');
 if(blockers.length)return freeze({version:HBU_VERSION,status:HBU_STATUS.HOLD,blockers,warnings:[],alternatives:[],selected:null});
 const results=[];let review=false;
 for(const a of alternatives){
  if(!a||!a.id||!a.legallyPermissible||!a.physicallyPossible){results.push({id:a&&a.id||'UNKNOWN',eligible:false,reason:'LEGAL_OR_PHYSICAL_GATE_FAILED'});continue;}
  const fields=['grossDevelopmentValueSar','hardCostsSar','softCostsSar','financeCostsSar','contingencySar','sellingCostsSar'];
  if(fields.some(k=>!finite(a[k])||a[k]<0)){return freeze({version:HBU_VERSION,status:HBU_STATUS.HOLD,blockers:[`INVALID_ALTERNATIVE_INPUT:${a.id}`],warnings:[],alternatives:results,selected:null});}
  ...
  results.push({id:a.id,eligible:true,financiallyFeasible,grossDevelopmentValueSar:a.grossDevelopmentValueSar,nonLandCostSar:nonLandCost,requiredProfitSar:requiredProfit,maximumLandBidSar,bidPerSqmSar});
 }
 const feasible=results.filter(r=>r.eligible&&r.financiallyFeasible).sort((x,y)=>y.maximumLandBidSar-x.maximumLandBidSar);
 if(!feasible.length)return freeze({version:HBU_VERSION,status:HBU_STATUS.REVIEW_REQUIRED,blockers:[],warnings:['NO_FINANCIALLY_FEASIBLE_HBU'],alternatives:results,selected:null,transactionAuthorized:false});
