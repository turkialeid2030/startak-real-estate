'use strict';
const DATED_RETURNS_VERSION='DATED_RETURNS_V3';
...
function solveDatedXirr({cashflows,tolerance=1e-10,npvToleranceSar=null,maxIterations=300,lowerBound=-.9999,upperBound=100}={}){
 ...
 if(Math.abs(fmid)>effectiveNpvTolerance)return freeze({version:DATED_RETURNS_VERSION,status:DATED_RETURNS_STATUS.HOLD,blockers:['XIRR_RESIDUAL_NPV_TOO_LARGE'],xirr:null,residualNpvSar:fmid,npvToleranceSar:effectiveNpvTolerance,iterations:i+1});
 return freeze({version:DATED_RETURNS_VERSION,status:DATED_RETURNS_STATUS.QUALIFIED,blockers:[],xirr:mid,residualNpvSar:fmid,npvToleranceSar:effectiveNpvTolerance,iterations:i+1,dayCount:'ACT/365.2425',semantics:'Dated XIRR is a mathematical return metric for the supplied cash flows; it is not an investment approval or forecast.'});
}
