...
const GOLDEN_VALIDATION_VERSION='GOLDEN_VALIDATION_V2';
...
function validateGoldenCorpus({cases,tolerancePct}={}){
  ...
  if(!finite(tolerancePct)||tolerancePct<0||tolerancePct>1) return freeze({version:GOLDEN_VALIDATION_VERSION,status:GOLDEN_STATUS.HOLD,blockers:['TOLERANCE_REQUIRED'],results:[]});
  const results=[];let review=false;const ids=new Set();
  for(const c of cases){
    if(!c||!nonEmptyString(c.id)||ids.has(c.id)||!finite(c.independentValueSar)||c.independentValueSar<=0||!finite(c.modelValueSar)||c.modelValueSar<=0||!nonEmptyString(c.sourceRef)){
    ...
    results.push({id:c.id,assetType:c.assetType||null,sourceRef:c.sourceRef.trim(),independentValueSar:c.independentValueSar,modelValueSar:c.modelValueSar,varianceSar,variancePct,withinTolerance});
  }
  return freeze({version:GOLDEN_VALIDATION_VERSION,status:review?GOLDEN_STATUS.REVIEW_REQUIRED:GOLDEN_STATUS.PASS,blockers:[],tolerancePct,results,semantics:'Golden validation measures model variance against independently sourced reference cases; it does not assert that either value is certified or universally correct.'});
}
