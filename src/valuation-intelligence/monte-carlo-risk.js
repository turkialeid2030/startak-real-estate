...
const MONTE_CARLO_VERSION = 'MONTE_CARLO_RISK_V5';
...
function solvePeriodicIrr(cashflows,{rateTolerance=1e-14,npvTolerance=null,maxIterations=400}={}){
  ...
  const scale=cashflows.reduce((s,v)=>s+Math.abs(v),0),effectiveNpvTolerance=npvTolerance===null?Math.max(1e-8,scale*1e-12):npvTolerance;
...
function runGovernedMonteCarlo({seed,iterations,base,distributions,thresholds}={}){
  ...
  if(base&&base.annualDebtServiceSar!==undefined&&base.annualDebtServiceSar!==null&&(!finite(base.annualDebtServiceSar)||base.annualDebtServiceSar<0))blockers.push('ANNUAL_DEBT_SERVICE_INVALID');
