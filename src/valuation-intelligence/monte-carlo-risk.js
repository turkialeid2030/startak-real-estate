...
const MONTE_CARLO_VERSION = 'MONTE_CARLO_RISK_V2';
...
function solvePeriodicIrr(cashflows,{tolerance=1e-10,maxIterations=300}={}){
  ...
  if(Math.abs(flo)<=tolerance)return lo;if(Math.abs(fhi)<=tolerance)return hi;
...
function runGovernedMonteCarlo({seed,iterations,base,distributions,thresholds}={}){
  ...
  for(let n=0;n<iterations;n++){
    ...
    npvs.push(npv);irrs.push(irr);if(debt)dscrs.push(minDscr);if(npv<0)npvNeg++;if(irr<thresholds.hurdleIrr)irrBreach++;if(debt&&minDscr<thresholds.minDscr)dscrBreach++;
  }
