...
function runGovernedMonteCarlo({seed,iterations,base,distributions,thresholds}={}){
  ...
  if(!thresholds||!finite(thresholds.hurdleIrr)||!finite(thresholds.minDscr)||!finite(thresholds.maxProbabilityNpvNegative)||!finite(thresholds.maxProbabilityDscrBreach))blockers.push('GOVERNED_THRESHOLDS_REQUIRED');
  ...
  for(let n=0;n<iterations;n++){
    ...
    if(!finite(g)||!finite(exitCap)||exitCap<=0||!finite(disc)||disc<=-1){invalid++;continue;}
    ...
    for(let y=1;y<=base.horizonYears;y++){
      ...
      npv+=(noi+terminal)/Math.pow(1+disc,y);
    }
    const terminalNoi=base.baseNoiSar*Math.pow(1+g,base.horizonYears);
    const terminalValue=terminalNoi/exitCap;
    const totalFuture=terminalValue+terminalNoi;
    const approxIrr=Math.pow(Math.max(totalFuture,0)/base.initialInvestmentSar,1/base.horizonYears)-1;
    npvs.push(npv);irrs.push(approxIrr);if(debt)dscrs.push(minDscr);
    if(npv<0)npvNeg++;if(approxIrr<thresholds.hurdleIrr)irrBreach++;if(debt&&minDscr<thresholds.minDscr)dscrBreach++;
  }
