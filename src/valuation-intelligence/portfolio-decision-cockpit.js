...
function unitInterval(v){return finite(v)&&v>0&&v<=1;}
function evaluatePortfolioDecision({assets,limits}={}){
 ...
 if(!limits||!unitInterval(limits.maxSingleAssetWeight)||!unitInterval(limits.maxCityWeight)||!unitInterval(limits.maxAssetTypeWeight)||!finite(limits.minPortfolioDscr)||limits.minPortfolioDscr<0)blockers.push('PORTFOLIO_LIMITS_REQUIRED');
 ...
 let total=0,totalNoi=0,totalDebtService=0;const cities={},types={},warnings=[];
 for(const a of assets){
  const debtProvided=a&&a.annualDebtServiceSar!==undefined;
  if(!a||!a.id||!finite(a.valueSar)||a.valueSar<=0||!finite(a.noiSar)||a.noiSar<0||!a.city||!a.assetType||(debtProvided&&(!finite(a.annualDebtServiceSar)||a.annualDebtServiceSar<0)))return freeze({version:PORTFOLIO_VERSION,status:PORTFOLIO_STATUS.HOLD,blockers:[`INVALID_PORTFOLIO_ASSET:${a&&a.id?a.id:'UNKNOWN'}`],warnings:[],metrics:null,transactionAuthorized:false});
  total+=a.valueSar;totalNoi+=a.noiSar;if(a.annualDebtServiceSar>0)totalDebtService+=a.annualDebtServiceSar;cities[a.city]=(cities[a.city]||0)+a.valueSar;types[a.assetType]=(types[a.assetType]||0)+a.valueSar;
 }
 ...
 const portfolioDscr=totalDebtService>0?totalNoi/totalDebtService:null;if(portfolioDscr!==null&&portfolioDscr<limits.minPortfolioDscr)warnings.push('PORTFOLIO_DSCR_BREACH');
