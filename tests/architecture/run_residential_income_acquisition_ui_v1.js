...
assert(api.includes("capabilityStatus: 'STRATEGIC_ASSET_INTELLIGENCE_V1'"));
...
assert(api.includes('calculateIncomeAnalysis(operatingCase, operatingMetrics, propertyCosts, readiness)'));
assert(api.includes('calculateAcquisitionBasis(operatingCase, propertyCosts, readiness)'));
assert(api.includes('calculateReverseUnderwriting(operatingCase, incomeAnalysis, acquisitionBasis, readiness)'));
assert(api.includes('calculateExitStrategyComparison(operatingCase, incomeAnalysis, acquisitionBasis, readiness)'));
assert(api.includes('calculateStrategicAssetIntelligence(operatingCase)'));
...
assert(panel.includes("t('riai.incomeAnalysis')"));
assert(panel.includes("t('riai.headlineMarkToMarket')"));
assert(panel.includes("t('riai.stabilizedNoi')"));
assert(panel.includes("t('riai.acquisitionBasis')"));
assert(panel.includes("t('riai.allInBasis')"));
assert(panel.includes("t('riai.equityBasis')"));
assert(panel.includes("t('riai.reverseUnderwriting')"));
assert(panel.includes("t('riai.maximumJustifiedPrice')"));
assert(panel.includes("t('riai.bindingConstraint')"));
assert(panel.includes("t('riai.exitStrategyComparison')"));
assert(panel.includes("t('riai.highestModeledNpvScenario')"));
assert(panel.includes("t('riai.exitRankingBoundary')"));
assert(panel.includes("t('riai.strategicAssetIntelligence')"));
assert(panel.includes("t('riai.currentLocationScore')"));
assert(panel.includes("t('riai.forwardAttractionScore')"));
assert(panel.includes("t('riai.subdivisionGate')"));
assert(panel.includes("t('riai.strategicBoundary')"));
