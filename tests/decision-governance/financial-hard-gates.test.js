...
assert.strictEqual(evaluateFinancialHardGates({ ...base, irrReliability: null }).status, 'INCOMPLETE');
assert.ok(evaluateFinancialHardGates({ ...base, irrReliability: 'UNKNOWN' }).failures.includes('IRR_RELIABILITY_INVALID'));
...
assert.ok(evaluateFinancialHardGates({ ...base, exitCapRate: 0 }).failures.includes('EXIT_CAP_RATE_INVALID'));
assert.ok(evaluateFinancialHardGates({ ...base, terminalValue: -1 }).failures.includes('TERMINAL_VALUE_NEGATIVE'));
assert.strictEqual(evaluateFinancialHardGates({ ...base, acquisitionBasis: null }).status, 'INCOMPLETE');
assert.ok(evaluateFinancialHardGates({ ...base, acquisitionBasis: 0 }).failures.includes('ACQUISITION_BASIS_INVALID'));
assert.strictEqual(evaluateFinancialHardGates({ ...base, holdingPeriod: null }).status, 'INCOMPLETE');
...
r = evaluateFinancialHardGates({ ...base, leverageEnabled: true, leveredNPV: 5, dscr: 1.3, minDscrThreshold: 1.25, loanAmount: null });
assert.strictEqual(r.status, 'INCOMPLETE');
assert.ok(r.incomplete.includes('LOAN_AMOUNT_MISSING'));
