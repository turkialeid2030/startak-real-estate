...
const single=evaluatePortfolioDecision({assets,limits:{...limits,maxSingleAssetWeight:.5}});
assert.strictEqual(single.status,PORTFOLIO_STATUS.REVIEW_REQUIRED);
assert.ok(single.warnings.includes('SINGLE_ASSET_CONCENTRATION_BREACH'));

const sameCity=assets.map((a)=>({...a,city:'Riyadh'}));
const city=evaluatePortfolioDecision({assets:sameCity,limits:{...limits,maxCityWeight:.8}});
...
const sameType=assets.map((a)=>({...a,assetType:'OFFICE'}));
const type=evaluatePortfolioDecision({assets:sameType,limits:{...limits,maxAssetTypeWeight:.8}});
...
const weakDebt=evaluatePortfolioDecision({assets:assets.map((a)=>({...a,annualDebtServiceSar:a.noiSar})),limits:{...limits,minPortfolioDscr:1.25}});
assert.strictEqual(weakDebt.status,PORTFOLIO_STATUS.REVIEW_REQUIRED);
assert.ok(weakDebt.warnings.includes('PORTFOLIO_DSCR_BREACH'));
