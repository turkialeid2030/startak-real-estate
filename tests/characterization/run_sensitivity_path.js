...
// Load the extracted function as a real module bound to the real engine and the
// governed assumption-model enum referenced by the production function body.
const moduleSrc = fnSource + '\nmodule.exports = { buildSensitivityData };';
const tmpPath = path.join(require('os').tmpdir(), `sens_${Date.now()}.js`);
fs.writeFileSync(
  tmpPath,
  `const { calculateInvestmentCase, STUDY_TYPE } = require(${JSON.stringify(path.join(__dirname, '..', '..', 'src', 'engines'))});\n` +
  `const { ASSUMPTION_MODEL_VERSION } = require(${JSON.stringify(path.join(__dirname, '..', '..', 'src', 'assumptions', 'assumption-model'))});\n` +
  moduleSrc,
);
...
const cases = [
  { mode: 'building', label: 'Existing Building / Unlevered', inputs: { ...gold['RE-GOLD-002_existing_building'].inputs, leverageEnabled: false }, studyType: STUDY_TYPE.EXISTING_BUILDING, assumptionModelVersion: ASSUMPTION_MODEL_VERSION.LEGACY },
  { mode: 'building', label: 'Existing Building / Levered', inputs: { ...gold['RE-GOLD-002_existing_building'].inputs, leverageEnabled: true }, studyType: STUDY_TYPE.EXISTING_BUILDING, assumptionModelVersion: ASSUMPTION_MODEL_VERSION.LEGACY },
  { mode: 'land', label: 'Land + Development / Unlevered', inputs: { ...gold['RE-GOLD-001_land_development'].inputs, leverageEnabled: false }, studyType: STUDY_TYPE.LAND_DEVELOPMENT, assumptionModelVersion: ASSUMPTION_MODEL_VERSION.LEGACY },
  { mode: 'land', label: 'Land + Development / Levered', inputs: { ...gold['RE-GOLD-001_land_development'].inputs, leverageEnabled: true }, studyType: STUDY_TYPE.LAND_DEVELOPMENT, assumptionModelVersion: ASSUMPTION_MODEL_VERSION.LEGACY },
];
...
for (const c of cases) {
  ...
  const rows = buildSensitivityData(c.mode, c.inputs, (key) => key, c.assumptionModelVersion);
  ...
  const canonical = calculateInvestmentCase({
    ...
    inputs: c.inputs,
    leverageEnabled: c.inputs.leverageEnabled,
    assumptionModelVersion: c.assumptionModelVersion,
  });
