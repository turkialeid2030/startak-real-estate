...
async function enterExplicitBuildingExitCap(page, value = '8.5') {
  ...
  const exitCap = page.getByText('معدل رسملة الخروج', { exact: true }).locator('xpath=ancestor::label[1]').locator('input').first();
  await exitCap.fill(value); await exitCap.blur(); await page.waitForTimeout(300); return exitCap;
}
...
try {
  ...
  const rootHtml = await page.locator('#root').innerHTML(); const dir = await page.locator('html').getAttribute('dir');
  record('E2E-01-BOOT', rootHtml.length > 100 && dir === 'rtl', `htmlLen=${rootHtml.length} dir=${dir}`);

  // New Deal is fail-closed: selecting an asset class must not synthesize a financial recommendation.
  await page.getByText('مبنى قائم', { exact: true }).click(); await page.waitForTimeout(250);
  const blankBuildingBody = await page.locator('body').innerText();
  ...
  const checkedAfter = await leverageButton.getAttribute('aria-checked'); const equityLabels = page.getByText('حقوق الملكية المطلوبة', { exact: true }); let visibleFinanceOutput = false;
  for (let i = 0; i < await equityLabels.count(); i++) if (await equityLabels.nth(i).isVisible()) { visibleFinanceOutput = true; break; }
  await leverageButton.click(); await page.waitForTimeout(300); const checkedFinal = await leverageButton.getAttribute('aria-checked');
  record('E2E-04-FINANCING', leverageCount === 1 && checkedBefore !== checkedAfter && checkedFinal === checkedBefore && visibleFinanceOutput, `count=${leverageCount} ${checkedBefore}->${checkedAfter}->${checkedFinal} visibleOutput=${visibleFinanceOutput}`);

  await page.getByTitle('الصفقات المحفوظة').click(); await page.waitForTimeout(250); const dealName = `صفقة-اختبار-${Date.now()}`;
  await page.getByPlaceholder('اسم الصفقة...').fill(dealName); await page.getByRole('button', { name: /^حفظ$/ }).click(); await page.waitForTimeout(500);
  const panelText = await page.locator('.fixed.inset-0.z-50').innerText(); record('E2E-05-SAVED-DEALS', panelText.includes(dealName), `deal=${dealName}`);
  const overlay = page.locator('.fixed.inset-0.z-50'); const innerPanel = overlay.locator('> div'); await innerPanel.locator('button').first().click(); await page.waitForTimeout(200);
  record('E2E-05B-DEALS-PANEL-CLOSE', (await overlay.count()) === 0);
  const resetButton = page.getByTitle(/استعادة القيم الأصلية لهذه الدراسة|التراجع عن التعديلات غير المحفوظة/); await resetButton.click(); await page.waitForTimeout(300);
  const resetBody = await page.locator('body').innerText(); record('E2E-06-RESET', !resetBody.includes('777777'));
  result.PAGE_ERRORS = pageErrors.length; result.FATAL_CONSOLE_ERRORS = consoleErrors.length; result.CORE_RUNTIME_NETWORK_FAILURES = httpErrors.length; result.HTTP_ERROR_RESPONSES = httpErrors; result.CONSOLE_ERROR_SAMPLES = consoleErrors.slice(0, 5);
  record('E2E-07-NO-PAGE-ERRORS', pageErrors.length === 0, JSON.stringify(pageErrors.slice(0, 3))); record('E2E-08-NO-CONSOLE-ERRORS', consoleErrors.length === 0, JSON.stringify(consoleErrors.slice(0, 3))); record('E2E-09-NO-HTTP-ERRORS', httpErrors.length === 0, JSON.stringify(httpErrors.slice(0, 3)));
} catch (error) { result.fatal_error = error.message; failed = true; }
finally { if (browser) await browser.close(); if (previewServer) await new Promise((resolve) => previewServer.httpServer.close(resolve)); fs.writeFileSync(`${EVIDENCE_DIR}/core-e2e-result.json`, JSON.stringify(result, null, 2)); console.log(JSON.stringify(result, null, 2)); if (failed) process.exitCode = 1; }
