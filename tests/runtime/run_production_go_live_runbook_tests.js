...
function e2fPacket(candidate = releaseCandidate(), overrides = {}) {
  const core = {
    schemaVersion: 1,
...
function e2iPacket(candidate = releaseCandidate(), { missing = [], ready = true } = {}) {
  const core = {
    schemaVersion: 1,
    readinessPacketId: 'e2i-production-readiness-001',
    upstreamCloseoutPacketId: 'e2h-production-closeout-001',
    upstreamCloseoutPacketHashSha256: 'a'.repeat(64),
...
(function testStageGapRejected() {
  const result = evaluateProductionGoLiveRunbook({ e2hCloseoutPacket: e2hPacket() });
...
(function testReleaseCandidateDriftRejected() {
  const e2f = e2fPacket();
  const e2g = e2gPacket(releaseCandidate({ sourceCommitSha: 'c'.repeat(40) }));
  const result = evaluateProductionGoLiveRunbook({
    e2fValidationPacket: e2f,
    expectedE2fValidationPacketHashSha256: e2f.validationPacketHashSha256,
    e2gDecisionPacket: e2g,
...
(function testE2hReadyForE2i() {
  const e2f = e2fPacket(); const e2g = e2gPacket(); const e2h = e2hPacket();
  const result = evaluateProductionGoLiveRunbook({
...
(function testE2iWaitingAndFinalStates() {
  const e2f = e2fPacket(); const e2g = e2gPacket(); const e2h = e2hPacket();
  const waiting = e2iPacket(releaseCandidate(), { missing: ['SAUDI_LEGAL_OPERATING_MODE_REVIEW'], ready: false });
...
(function testCliStatusAndHardening() {
  ...
  const e2f = e2fPacket(); const e2g = e2gPacket(); const e2h = e2hPacket(); const e2i = e2iPacket();
  const files = {
  ...
  const link = path.join(dir, 'link.json'); fs.symlinkSync(files.e2f, link);
